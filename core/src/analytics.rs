...
impl TimeBucket {
    ...
    fn merge_block(&mut self, block: &BlockSummary) {
        ...
        for (addr, count) in &block.sender_counts {
            *self.sender_counts.entry(addr.clone()).or_default() += count;
        }
        for (addr, count) in &block.receiver_counts {
            *self.receiver_counts.entry(addr.clone()).or_default() += count;
        }
        for (proto, count) in &block.protocol_counts {
            *self.protocol_counts.entry(*proto).or_default() += count;
        }
        for (proto, mass) in &block.protocol_mass {
            *self.protocol_mass.entry(*proto).or_default() += mass;
        }
        for (proto, fees) in &block.protocol_fees {
            *self.protocol_fees.entry(*proto).or_default() += fees;
        }
...
impl AnalyticsEngine {
    ...
    /// Compute an aggregated view for a given time window.
...
fn build_aggregated_view<'a>(items: impl Iterator<Item = AggregateItem<'a>>) -> AggregatedView {
    ...
    for item in items {
        ...
        for (addr, count) in item.sender_counts {
            *sender_totals.entry(addr.clone()).or_default() += count;
        }
        for (addr, count) in item.receiver_counts {
            *receiver_totals.entry(addr.clone()).or_default() += count;
        }
        for (proto, count) in item.protocol_counts {
            *protocol_totals.entry(*proto).or_default() += count;
        }
        for (proto, mass) in item.protocol_mass {
            *protocol_mass_totals.entry(*proto).or_default() += mass;
        }
        for (proto, fees) in item.protocol_fees {
            *protocol_fee_totals.entry(*proto).or_default() += fees;
        }
